...
interface GCalState {
  ...
  createEvent: (input: { summary: string; location?: string; description?: string; startDateTime?: string; endDateTime?: string; allDayDate?: string; allDayEndDate?: string; attendees?: string[]; taskId?: string; transparency?: 'opaque' | 'transparent'; timeblock?: boolean; noteRef?: string }) => Promise<string | null>
...
export const useGCalStore = create<GCalState>((set, get) => ({
   ...
   */
  createEvent: async ({ summary, location, description, startDateTime, endDateTime, allDayDate, allDayEndDate, attendees, taskId, transparency, timeblock, noteRef }) => {
    const { calendars } = get()
    ...
    try {
      const created = await createCalendarEvent(token, { calendarId: target, summary, location, description, startDateTime, endDateTime, allDayDate, allDayEndDate, attendees, taskId, transparency, timeblock, noteRef })
