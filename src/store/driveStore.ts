...
export const useDriveStore = create<DriveState>((set, get) => ({
  ...
  connect: async () => {
    ...
    try {
      ...
      set({ token: granted.token, expiry, wasConnected: true, connecting: false, error: null })
      return true
    } catch (e) {
  ...
  disconnect: () => {
    ...
    set({ token: null, expiry: null, wasConnected: false, meta: {}, error: null })
  },
   ...
   */
  ensureToken: async () => {
    const { token, expiry, wasConnected } = get()
    if (token && expiry && expiry > Date.now()) return token
    if (!wasConnected || !GIS_CONFIGURED) return null
    try {
      const granted = await requestGoogleToken({
        scope: DRIVE_SCOPE,
        interactive: false,
  ...
  search: async (query, folderId) => {
    ...
    try {
      ...
      set({ error: e instanceof Error ? e.message : '드라이브 검색 오류' })
      return []
    }
  },
