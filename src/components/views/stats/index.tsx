...
import { CAT_COLORS, STATUS_COLORS, CATEGORIES, STATUS_LIST, MEMBERS } from '../../../types'
import type { Category, Status, MemberKey } from '../../../types'
...
export function StatsView() {
  ...
  const overdue = tasks.filter(t => t.due && t.status !== '완료' && new Date(t.due) < new Date(new Date().toDateString())).length
      ...
      <div className="flex flex-col gap-[16px]">
        ...
        <div className="grid grid-cols-2 gap-[16px]">
          ...
          <StatSection title="📂 카테고리별">
            {CATEGORIES.map(cat => {
              const cnt = tasks.filter(t => t.cat === cat).length
              const c = CAT_COLORS[cat]
              return (
                <BarRow key={cat} label={cat} count={cnt} max={total} bg={c.bg} fill={c.text} />
              )
            })}
          </StatSection>
          ...
          <StatSection title="🔄 상태별">
            {STATUS_LIST.map(status => {
              ...
              return (
                <BarRow key={status} label={status} count={cnt} max={total} bg={c.bg} fill={c.text} />
              )
            })}
          </StatSection>
        </div>
        ...
        <StatSection title="👤 담당자별">
          <div className="grid grid-cols-3 gap-[16px]">
            {(Object.keys(MEMBERS) as MemberKey[]).map(key => {
              ...
              const myDone = myTasks.filter(t => t.status === '완료').length
...
function BarRow({ label, count, max, bg, fill }: {
  label: string; count: number; max: number; bg: string; fill: string
}) {
  return (
    <div className="flex items-center gap-[10px] mb-[9px]">
      ...
      <div className="flex-1 h-[8px] bg-gray-100 rounded-full overflow-hidden">
        <div
          ...
          style={{ width: max ? `${(count / max) * 100}%` : '0%', background: fill }}
        />
      </div>
