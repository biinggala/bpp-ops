...
| `update_project` | 이름·색·마감·고객사·선반 (**보관은 빠졌습니다** — 아래) |
| `list_project_links` | 프로젝트 자체에 걸린 자료 (계약서, 브랜드 가이드 등) |
| `add_project_link` / `remove_project_link` | 프로젝트 자료 추가/해제 |

**프로젝트를 만드는 도구는 없습니다. 보관하는 길도 없습니다.** 있었고, 뺐습니다.

블랙페이퍼가 노션을 협업 도구로 쓰다 접은 주된 이유가 각자 자기 데이터베이스를
만들어서 정보가 안 모인 것이었습니다. 같은 일이 여기서 **더 조용히** 일어납니다 —
커넥터로 만든 프로젝트는 멤버가 만든 사람 하나뿐이라 아무도 못 봅니다. 못 보면
'그거 이미 있는데요'라고 말해 줄 수도 없습니다. 노션에서는 적어도 보였습니다.

이름이 안 닮아도 중복입니다 — '로고 시안'과 'BI 개편'이 같은 일일 수 있습니다.
그래서 비슷한 이름을 막는 식으로는 안 풀리고, 그 일이 어디에 속하는지 아는
사람이 정해야 합니다.

업무를 더하는 것과는 무게가 다릅니다. 업무는 이미 합의된 그릇 안에 들어가고,
틀리면 옮기면 됩니다. 프로젝트는 오십 명이 '여기 무슨 일이 있나'를 읽는 목록에
한 줄을 더하는 일이고, 그 한 줄이 썩는 것은 몇 달 뒤에나 보입니다.

맞는 프로젝트가 없으면 `create_task`를 `project_id` 없이 부르면 됩니다 —
그 사람의 개인 업무로 갑니다.

보관도 같은 이유로 뺐습니다. 보관은 그 프로젝트를 **모두의 사이드바에서
내리고** 전체 업무·내 할 일·통계에서 빼는 일입니다. 한 사람이 자기 화면을
정리하는 것이 아니라 오십 명이 읽는 목록을 바꾸는 것이고, 그 일이 정말
끝났는지 아는 사람이 정해야 합니다.

이름·색·마감·선반은 남아 있습니다. 그것들은 틀리면 눈에 보이니 누구든
되돌립니다. **보관은 안 보이게 되는 것이라 틀린 것도 안 보입니다.**
...
브라우저가 받는 구글 토큰은 **한 시간짜리**입니다. 갱신할 열쇠(refresh token)는
클라이언트 비밀을 쥔 쪽만 바꿔 쓸 수 있어서 브라우저는 가질 수 없고, 그래서
지금까지는 한 시간마다 구글에 조용히 다시 물었습니다. 그 갱신은 **구글 세션이
살아 있고 브라우저가 허락할 때만** 됩니다 — 사파리와 아이폰은 그 자리를 막아서,
아무것도 안 했는데 몇 시간마다 '재연동'이 떴습니다.

이 서버가 열쇠를 들면 그 문제가 없어지고, 연결이 기기가 아니라 **사람**에게
붙습니다. 노트북에서 한 번 하면 폰에서도 됩니다.

| 자리 | 무엇 | 누가 읽나 |
|---|---|---|
| `googleAuth/{uid}` | refresh token과 허락받은 범위 | **아무도.** 규칙이 닫아 둡니다 |
| `googleLinked/{uid}` | '연결됨'과 범위 | 본인만 |

### 켜는 순서 — 이 순서를 지켜야 합니다

1. 구글 클라우드 콘솔 → API 및 서비스 → 사용자 인증 정보 → 커넥터가 쓰는
   OAuth 클라이언트 → **승인된 리디렉션 URI**에 추가:

       https://<배포주소>/google/callback

   (커넥터 로그인이 쓰는 `/oauth/google/callback`과 **다른 주소**입니다.
   둘 다 있어야 합니다. 클라이언트를 새로 만들지 마세요 — 이미 붙어 있는
   팀원들의 커넥터가 전부 끊깁니다.)

2. 저장소 → Settings → Variables에 `GOOGLE_LINK_ENABLED = 1`

3. 커넥터 배포(Actions → Deploy MCP server)

**1번 없이 2번을 하면** 사람이 연결을 누를 때 `redirect_uri_mismatch`만
봅니다. 그래서 스위치가 따로 있습니다 — 켜기 전까지 이 길은 전부 503이고,
앱은 예전처럼 브라우저가 직접 토큰을 받습니다.

`/google/health`가 지금 켜져 있는지와 리디렉션 주소를 그대로 답합니다.

### 길

| 길 | 하는 일 |
|---|---|
| `POST /google/start` | 동의 주소 한 장. 창은 앱이 엽니다 |
| `GET /google/callback` | 열쇠를 받아 보관하고, 사람에게 '닫으세요' 한 장 |
| `POST /google/token` | 한 시간짜리 토큰 한 장. 허락 안 받은 범위는 412 |
| `POST /google/disconnect` | 범위를 주면 그것만, 안 주면 통째로 |
